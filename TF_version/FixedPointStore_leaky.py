...
sys.path.insert(0,'C:/Users/Nishant Joshi/Downloads/DDN-public/rnn_flip_flops/')
...
class FixedPointStore:
  ...
  def __init__(self, 
    ...
    if alloc_zeros == True:
      if self.ctype=='ESN':
        ...
        self.dtype = dtype
        self.xstar = self._alloc_zeros([num_inits, num_states, num_delays])
        self.x_init = self._alloc_zeros([num_inits, num_states, num_delays])
        self.inputs = self._alloc_zeros([num_inits, num_inputs, num_delays])
        self.F_xstar = self._alloc_zeros([num_inits, num_states, num_delays])
        ...
        self.eigval_J_xstar = self._alloc_zeros((num_inits, num_states, num_delays))
      ...
      if self.ctype=='ESN':
        ...
        if num_inits is None:
          if xstar is not None:
            self.num_inits, self.num_states,self.delays = xstar.shape
          elif x_init is not None:
            self.num_inits, self.num_states,self.delays = x_init.shape
          elif F_xstar is not None:
            self.num_inits, self.num_states,self.delays = F_xstar.shape
  ...
  def get_unique(self):
    ...
    # idx_keep = []
    # idx_checked = np.zeros(self.num_inits, dtype=np.bool)
    # for idx in range(self.num_inits):
  ...
  def decompose_jacobians(self, do_batch=True, str_prefix=''):
    ...
    self.eigvec_J_xstar = self._alloc_zeros((num,num_states, num_states),dtype=np.complex64)
