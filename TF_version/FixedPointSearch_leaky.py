...
sys.path.append('C:/Users/Nishant Joshi/Downloads/DDN-public/rnn_flip_flops/fixed_point_edits')
import absl
...
class FixedPointSearch:
  ...
  def __init__(self, 
    ...
    assert opt_type in ['joint','seq'], 'opt type shold be either joint or seq'
    self.n_iters_per_print_update = 100
  ...
  def build_vars(self, init_states):
    ...
    if self.ctype == 'ESN':
      if self.opt_type=='seq':
        n_state,n_delay = init_states.shape
        x = tf.Variable(np.concatenate([init_states,np.zeros((1,n_delay))],axis=0),dtype=tf.float32)
        x_rnn_cell = x

      else:
        n_time,n_state,n_delay = init_states.shape
        init_states = np.concatenate([init_states,np.zeros((n_time,1,n_delay))],axis=1)
  ...
  def compute_input_jacobians(self, fps):
    def grab_RNN_for_dFdu(initial_states, inputs):
      ...
      init = tf.compat.v1.variables_initializer(var_list = [x, inputs])
      self.sess.run(init)
  ...
  def sample_states_DDN(self, init_size, state_matrix, noise):


    matrix = state_matrix
    
    [n_time, n_states,  delay] = matrix.shape

    valid_idx = np.ones((n_time), dtype=np.bool)

    time_idx = np.nonzero(valid_idx)[0]

    
    sample_indices = nrand.RandomState(200).randint(0, high = len(time_idx), size = [init_size])

    states = np.zeros([init_size, n_states, delay])

    for i in range(len(sample_indices)):
      # init_idx = trial_idx[i]
      t_idx = sample_indices[i]
      states[i,:,:] = matrix[t_idx,:,:]
  ...
  def identify_distance_non_outliers(self, fps, initial_states, dist_thresh):
    ...
    if self.ctype == 'ESN':
        n_time,n_state,n_delay = initial_states.shape
        initial_states = np.concatenate([initial_states,np.zeros((n_time,1,n_delay))],axis=1)
  ...
  def run_iteration_loops(self, states, inputs, init_array):
    ...
    q_prev_tf = tf.compat.v1.placeholder(tf.float32, shape=list(q.shape), name='q_prev')
    ...
    grad_norm_clip_val = tf.compat.v1.placeholder(tf.float32, name='grad_norm_clip_val')
    ...
    optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate, **self.adam_optimizer_hps)
    ...
    if self.ctype == 'ESN':
      if self.opt_type == 'seq':  
        fixed_point = FixedPointStore(xstar       = np.expand_dims(ev_x[:-1,:],axis=0),
                                      inputs      = inputs,
                                      dtype       = self.dtype,
                                      alloc_zeros = False, 
                                      x_init      = np.expand_dims(states,axis=0),
                                      F_xstar     = np.expand_dims(ev_F[:-1,:],axis=0), 
                                      qstar       = ev_q,
                                      dq          = ev_dq,
                                      n_iters     = iter_count,
                                      opt         = self.opt_type
  ...
  def run_joint_optimization(self, initial_states, inputs):
      ...
      q_prev_tf = tf.compat.v1.placeholder(tf.float32, 
                                shape=list(q.shape), 
                                name='q_prev')
  ...
  def run_sequential_optimization(self, states, inputs, q_prior = None):
    ...
    if self.ctype  == 'ESN':
      fps = FixedPointStore(num_inits=num_inits, num_states=num_states, num_inputs=num_inputs,num_delays=states.shape[-1], alloc_zeros=True)
  ...
  def find_fixed_points(self, inputs, save=False):
    ...
    if self.ctype == 'LSTM':
      ...
      n = self.sampled_states.shape[0]  #time idx
      _state = self.sampled_states
